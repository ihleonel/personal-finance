...
export function CategorySummaryTable({ summary }: CategorySummaryTableProps) {
    ...
    <div className="overflow-x-auto">
      <Table>
        ...
        <TableBody>
          ...
          {summary.rows.length > 0 ? (
            <TableRow data-testid="cat-totals-row" className="border-t-2">
              <TableCell className="sticky left-0 bg-background font-semibold">
                Total
              </TableCell>
              {summary.totals.amounts.map((amt, i) => {
                const value = Number(amt)
                const display = value === 0 ? "—" : formatAmount(amt)
                const color =
                  value === 0
          ...
          {summary.rows.length > 0 ? (
            <TableRow data-testid="cat-accumulated-row">
              <TableCell className="sticky left-0 bg-background font-semibold">
                Saldo acumulado
              </TableCell>
              {summary.totals.accumulated.map((amt, i) => {
                const value = Number(amt)
                const display = value === 0 ? "—" : formatAmount(amt)
                const color =
                  value === 0
